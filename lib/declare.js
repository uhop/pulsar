define(["./lang"], function(lang){
    //TODO: do we include it in the base or provide as a standalone module?
    //TODO: the only external function needed is lang.mixin(), should we eliminate it?

    // I removed the legacy support and ensured that it can be used in the strict mode.
	...
	function inherited(args, a, f){
        ...
        caller = opts.call(args) == "[object Function]" ? args : args.callee;
		f = 0;
	...
	// single constructor
	function singleConstructor(ctor){
		return function(){
			if(!(this instanceof arguments.callee)){
				// not called via new, so force it
				return applyNew(arguments);
	...
	function declare(className, superclass, props){
		...
		bases[0] = ctor = (chains && chains.constructor === "manual") ? simpleConstructor(bases) :
			(bases.length == 1 ? singleConstructor(props.constructor) : chainedConstructor(bases));
